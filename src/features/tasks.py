...
def parse_tasks(content: str) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        parts = content.split('---')
        if len(parts) < 4:
            logger.warning("Invalid format: need at least 3 '---' delimiters for tasks section")
            return tasks
...
def toggle_task(filepath: Path, task_index: int) -> bool:
    ...
    try:
        ...
        if task_index < 0 or task_index >= len(tasks):
            logger.error(f"Invalid task index: {task_index} (total tasks: {len(tasks)})")
            return False
...
def add_task(filepath: Path, task_text: str) -> bool:
    ...
    try:
        ...
        parts = content.split('---')
        if len(parts) < 4:
            logger.error(f"Invalid format in {filepath}: need at least 3 '---' delimiters")
            return False
